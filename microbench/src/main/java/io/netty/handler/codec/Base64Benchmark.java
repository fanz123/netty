...
public class Base64Benchmark extends AbstractMicrobenchmark {
    private final ResourceLeakDetector.Level oldLevel = ResourceLeakDetector.getLevel();
    ...
    @Param({ "DISABLED", "SIMPLE", "ADVANCED", "PARANOID" })
    public String level;
    ...
    private static ByteBuf encodeJava8(ByteBufAllocator allocator, ByteBuf src) {
        ByteBuffer srcBuffer = src.internalNioBuffer(src.readerIndex(), src.readableBytes());
        int remaining = srcBuffer.remaining();
        ByteBuffer encodedBuffer = java.util.Base64.getMimeEncoder(76, new byte[]{ '\n' }).encode(srcBuffer);
